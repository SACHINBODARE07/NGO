        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7 space-y-10 mt-8 md:mt-10">
            <header className="space-y-6">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-teal-950 tracking-tighter leading-tight">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <div key={i} className="bg-white p-8 rounded-[2rem] border border-zinc-100 shadow-sm flex items-start gap-4">
                  <div className="w-12 h-12 bg-teal-50 rounded-xl flex items-center justify-center text-teal-600 shrink-0">
                    <item.icon size={24} />
                  </div>
                  <div>
                    <h4 className="font-black text-teal-950">{item.title}</h4>
                ...
                {givingLevels.map((level) => (
                  <button
                    ...
                    className={`p-6 rounded-[2rem] border-2 transition-all text-left space-y-2 ${
                      ...
                      ? "border-rose-600 bg-rose-50 ring-4 ring-rose-600/10" 
                      : "border-white bg-white hover:border-zinc-200"
                    }`}
                  >
                    <span className="text-2xl font-black text-teal-950">₹{level.amount}</span>
                    <p className="text-xs font-black text-rose-600 uppercase tracking-tighter">{level.label}</p>
                    <p className="text-[10px] text-zinc-400 font-medium leading-tight">{level.desc}</p>
                  </button>
                ))}
              </div>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-12 pr-6 font-black text-xl focus:outline-none focus:border-rose-600 transition-all placeholder:text-white/20"
                    />
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-white/10">
                    ...
                    <button className="w-full bg-rose-600 hover:bg-rose-700 py-5 rounded-2xl font-black text-lg shadow-xl shadow-rose-900/40 transition-all flex items-center justify-center gap-3">

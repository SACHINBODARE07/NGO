  ...
  return (
    <header className={`fixed top-0 w-full z-[100] transition-all duration-300`}>
      ...
      <div className={`bg-teal-950 text-zinc-300 py-1.5 hidden md:block border-b border-white/5 transition-all duration-500 ${isScrolled ? "h-0 opacity-0 overflow-hidden" : "h-auto opacity-100"}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-xs md:text-sm font-bold uppercase tracking-wider">
          <div className="flex gap-6">
            <a href="tel:+919131931137" className="flex items-center gap-1.5 hover:text-rose-400 transition-colors"><Phone size={14} /> +91 91319 31137</a>
      ...
      <nav className={`transition-all duration-300 ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-md py-1.5" : "bg-white py-2.5"} border-b border-zinc-100`}>
        <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center">
          ...
          {/* Logo (already increased) */}
          <Link href="/" className="relative h-20 w-20 md:h-28 md:w-28 hover:scale-105 transition-transform">
          ...
          <div className="hidden lg:flex items-center gap-0.5">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className={`px-3 py-1.5 rounded-full text-sm md:text-base font-extrabold uppercase tracking-wider transition-all ${
